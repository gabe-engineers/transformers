...
class HqqHfQuantizer(HfQuantizer):
    ...
    def _setup_missing_key_filters(self, model, checkpoint_files):
        ...
        for ckpt_file in checkpoint_files:
            if ckpt_file.endswith(".safetensors"):
                ...
                state_dict = self._load_torch_checkpoint(ckpt_file)
                state_dict = self._flatten_nested_state_dict(state_dict)
                for k in state_dict:
                    if k.endswith(".W_q"):
                        quantized_modules.add(k[: -len(".W_q")])
        ...
        if quantized_modules:
            ...
            existing = set(model._keys_to_ignore_on_load_missing or [])
            model._keys_to_ignore_on_load_missing = existing | set(escaped)
    ...
    def _load_torch_checkpoint(self, ckpt_file):
        try:
    ...
    def _flatten_nested_state_dict(self, state_dict):
    ...
    def _normalize_nested_checkpoint_files(self, checkpoint_files):
        ...
        for ckpt_file in checkpoint_files:
            if ckpt_file.endswith(".safetensors"):
                normalized_files.append(ckpt_file)
                continue
            ...
            temp_file = os.path.join(temp_dir, os.path.basename(ckpt_file))
    ...
    def _process_model_before_weight_loading(
        ...
        **kwargs,
        ...
        if self.pre_quantized:
            ...
            checkpoint_files = self._normalize_nested_checkpoint_files(checkpoint_files)
            ...
            existing = set(model._keys_to_ignore_on_load_unexpected or [])
            model._keys_to_ignore_on_load_unexpected = existing | set(ignore_unexpected)
    ...
    def _load_hqq_from_checkpoint(self, model: "PreTrainedModel"):
        ...
        for ckpt_file in self._checkpoint_files:
            if ckpt_file.endswith(".safetensors"):
                ...
                full_state_dict.update(self._flatten_nested_state_dict(self._load_torch_checkpoint(ckpt_file)))
